...
def calc_tile_locations(tile_size, image_size):
    ...
    """
    image_size_y, image_size_x = image_size#375,1242
    locations = []
...
def pooled_class_centroids_all(items, num_classes, id2trainid, tile_size=1024):
    ...
    centroids = defaultdict(list)
    new_centroids = pool.map(class_centroids_item, items)#把可迭代对象items一个一个的传入class_centroids_item中
    pool.close()
    pool.join()
...
def build_epoch(imgs, centroids, num_classes, class_uniform_pct):
    ...
    num_per_class = int((num_epoch * class_uniform_pct) / num_classes)#4
    num_rand = num_epoch - num_per_class * num_classes#103
    # create random crops
