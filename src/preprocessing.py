...
class Rescale:
    ...
    def __call__(self, sample): #__call__可以实现调用功能
...
def onehot_to_binary_edges(mask, radius, num_classes):
    ...
    edgemap = np.zeros(mask.shape[1:])#(480,640)
...
class ToEdge:
    def __call__(self, sample):
        ...
        _edgemap = onehot_to_binary_edges(_edgemap, 2, num_classes).astype('float32')#(1,480,640)通道为1.
